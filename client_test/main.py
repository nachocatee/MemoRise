...
ROOT = os.path.dirname("C:/Users/SSAFY/Desktop/streamlit test/client_test/")
...
class VideoTransformTrack(VideoStreamTrack):
    ...
    async def recv(self):
        ...
        #             # 라벨 그리기
        #             cv2.putText(img, label, (label_x, label_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)    
    ...
    def on_datachannel(channel):
        ...
        def on_message(message):
            if channel.label == "chat":
                text = message.strip()
                log_info("Received chat message: %s", text)
            else:
                log_info("Received message on channel %s", channel.label)
