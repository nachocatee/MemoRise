...
dimension = 512  # SiameseNetwork에서 출력되는 벡터의 차원
...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(1000, 512)  # 수정: 차원을 512로 변경
...
if os.path.exists('siamese_512_weights.pth'):
    model_siamese.load_state_dict(torch.load('siamese_512_weights.pth'))
...
# 로컬 이미지 처리
local_image = cv2.imread('C:/Users/SSAFY/Desktop/yolotest/starbucks Tumbler.jpg')
...
while True:
    ...
    if cv2.waitKey(1) & 0xFF == ord('s'):
        torch.save(model_siamese.state_dict(), 'siamese_512_weights.pth')
        print("Weights saved!")
